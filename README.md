...
초기 인사이트를 실시간으로 집계합니다. 5점 척도는 부정·중립·긍정 양방향
Likert 막대와 평균을 함께 보여 주며, `전혀 그렇지 않다`·`그렇지 않다`·
`보통이다`·`그렇다`·`매우 그렇다`를 파스텔 빨강·분홍·회색·하늘색·파랑으로
구분합니다. `보통이다` 구간의 가운데를 0% 기준선으로 고정해 부정 응답은 왼쪽, 긍정 응답은
오른쪽으로 뻗는 연속형 누적 막대로 표시합니다.
서술형 문항은 2명 이상이 공통으로 사용한
단어만 워드클라우드와 키워드 순위로 표시합니다. 원본 응답 대신 집계 결과만
브라우저로 전달됩니다.
